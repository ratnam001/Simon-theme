...
?>
	...
	<?php if(count($blocks) < LOF_MODULE_ADVANCE_CUSTOM_LIMIT_BLOCK){ ?>
		<a onclick="return false;" class="lofaddnew-block-item lofaddnew-block-<?php echo $id_position;?>" href="<?php echo $link->getModuleLink("lofadvancecustom","popup",array('fc'=>'module','id_shop'=>(int)($this->context->shop->id),'id_position'=>$id_position,'addBlock'=>1,'bo_theme'=>$employee->bo_theme,'id_divposition'=>'lofposition-'.$id_position,'nb'=>count($blocks),'token'=>Tools::getValue('token'),'secure_key'=>$this->secure_key));?>" title="add"><img border="0" src="../img/admin/add.gif"/><?php echo $this->l('Add new block');?></a><br/>
	<?php
	}
	if($blocks){
		foreach($blocks as $key=>$b){
