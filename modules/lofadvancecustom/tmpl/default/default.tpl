<div id="lofadvafooter{$pos}" class="lofadvafooter">
	{foreach from=$lofpositions item=blocks key=key_pos}
		<div id="lofadva-pos-{$key_pos + 1}" class="lof-position">
			<div class="lof-position-wrap">
			{foreach from=$blocks item=block key=key_block}
				{if $key_block == 0}
					{assign var="custom_class_toogle" value="toggle-footer"}
					{assign var="custom_class" value="col-xs-12 col-md-3 col-sm-3"}
				{elseif $key_block == 1}
					{assign var="custom_class_toogle" value="toggle-footer"}
					{assign var="custom_class" value="col-xs-12 col-md-3 col-sm-3"}
				{elseif $key_block == 2}
				...
				{elseif $key_block == 3}
					...
					{assign var="custom_class" value="col-xs-12 col-md-3 col-sm-3"}
				{/if}				
				<section class="{$custom_class} {if $block.show_title}footer-block{/if} {if $key_block == 3}footer-block{/if}">
					{if $block.show_title}
						<h4>{$block.title}</h4>
					{elseif $key_block == 3}
						<h4 class="hidden-lg hidden-sm">{l s='Contact Address' mod='lofadvancecustom'}</h4>
					{/if}
					<ul class="{$custom_class_toogle}">
